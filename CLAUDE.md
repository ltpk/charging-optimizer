...
src/
  ...
  components/
    Sidebar.tsx            — all controls; NumField uses defaultValue+key pattern (uncontrolled). Everyday controls (Battery State sliders, Charging Plan = mode/window/charge-by) stay visible; set-once config (Vehicle capacity/loss/power, Transfer Fee, Margins, Solar PV) lives in a collapsed "Advanced Setup" Accordion. `InfoTip` (InfoOutlined + MUI Tooltip) annotates non-obvious fields; charging mode is a One block / Cheapest slots ToggleButtonGroup (not a checkbox). On md+ the sidebar renders inline; on xs/sm it's a temporary Drawer toggled by the AppBar settings button (App.tsx builds the `sidebar` element once and renders it in either an inline Box or a Drawer)

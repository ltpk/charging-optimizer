...
const PREDICT_URL      = 'https://raw.githubusercontent.com/vividfog/nordpool-predict-fi/refs/heads/main/deploy/prediction.json'

interface StoredEntry extends Omit<PriceEntry, 'dt'> { dtIso: string }
interface SpotHintaRow { DateTime: string; PriceWithTax: number }
...
async function fetchActualSpot(): Promise<PriceEntry[]> {
  ...
  const [todayRes, forwardRes] = await Promise.allSettled([
    fetch(SPOT_TODAY_URL).then(r => r.ok ? r.json() as Promise<SpotHintaRow[]> : []).catch(() => [] as SpotHintaRow[]),
    fetch(SPOT_FORWARD_URL).then(r => r.ok ? r.json() as Promise<SpotHintaRow[]> : []).catch(() => [] as SpotHintaRow[]),
  ])
  ...
  raw.filter(d => d.DateTime != null).forEach(d => {
    ...
    if (entry) { entry.total += d.PriceWithTax; entry.count++ }

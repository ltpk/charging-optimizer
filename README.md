...
- Metrics panel shows hours needed (with kWh to be drawn from the grid), rounded duration, estimated total cost with average c/kWh for the optimal period, and the estimated charging completion time ("Done by"); when solar is enabled it also shows current solar output, the % of the charge covered by solar, and the estimated € saved vs. grid-only
- Cheapest-hours list bars use an absolute scale — length and color (green → amber → red) are normalized against all upcoming hours, so a full green bar means genuinely cheap, not just cheapest among the picked hours

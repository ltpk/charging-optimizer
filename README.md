...
- Metrics panel shows a "Charge plan" box (hours needed, with rounded duration, kWh to be drawn from the grid, and the estimated completion time "done by") and estimated total cost with average c/kWh for the optimal period; when solar is enabled it also shows current solar output, the % of the charge covered by solar, and the estimated € saved vs. grid-only

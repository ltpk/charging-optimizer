...
- Metrics panel shows a "Charge plan" box (hours needed, with rounded duration, kWh to be drawn from the grid, the estimated completion time "done by", and — when solar is enabled — the % of the charge covered by solar and estimated € saved vs. grid-only, both scoped to the recommended charging hours), estimated total cost with average c/kWh for the optimal period, and a "Spot now" box with the current slot's spot price, transfer fee, and (when solar is enabled) forecast solar output
